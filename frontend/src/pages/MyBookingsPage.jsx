...
import { Calendar, Clock, MapPin, XCircle, CheckCircle, Clock5, AlertCircle, Info, LayoutList, CalendarDays } from 'lucide-react';
...
const MyBookingsPage = () => {
    ...
    const fetchBookings = async () => {
        try {
            ...
            const eventResponse = await bookingService.getEvents();
            setCalendarEvents(eventResponse.data);
        } catch (error) {
                            ...
                            <FullCalendar
                                ...
                                eventClick={(info) => {
                                    if(info.event.title !== "Booked") {
                                        alert(`Your Booking: ${info.event.title}\nStart: ${info.event.start.toLocaleString()}\nEnd: ${info.event.end.toLocaleString()}`);
                                    }
                                }}
                                dateClick={(info) => {
                                    alert(`You clicked date: ${info.dateStr}\nGo to Resource Gallery to book!`);
                                    navigate('/resources');
                                }}
                            />
                        </div>
